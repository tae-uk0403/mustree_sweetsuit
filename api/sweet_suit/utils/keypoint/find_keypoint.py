...
def find_keypoint2(task_folder_path, body_position, model_name, model_position, image_name):


    model = pose_hrnet.get_pose_net(is_train=False).cuda()
    pretraind_model = f"api/sweet_suit/removed_bg_model/{body_position}/{model_position}.pth"
    model.load_state_dict(torch.load(pretraind_model), strict=True)

    c, s, r = np.array([719.5, 959.5]), np.array([11.368751, 15.158334]), 0  # center, scale, rotation
    # 이미지 크기에 맞는 [1920, 1440] c, s, r 이다.
    if body_position == 'lower':
        c, s, r = np.array([719.5, 1440]), np.array([5, 6.6666]), 0  # center, scale, rotation
